    ...
    if (window.confirm("Are you sure you want to delete this chat?")) {
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_BACKEND_URL}/api/chat/del/${chatId}`,
          { userId },
              ...
              {chats?.slice().reverse().map((chat) => (
                <li key={chat.chatId} className="w-full flex items-center">
                  <button
                    ...
                    className={`flex items-center gap-2 p-2 w-[80%] transition-all duration-300 ease-in-out ${
                      chat.chatId == chatId ? "bg-[#555] text-white rounded-3xl" : "hover:bg-[#333] hover:rounded-3xl"
                    }`}
                  >
                    <IoChatboxOutline />
